...
public class XmppConnectionTest extends EasyMockSupport {
	...
	public void testApproveSubscription() throws Exception {
		...
		roster.createEntry(eq(TEST_CONTACT), eq(TEST_CONTACT_NAME), aryEq(new String[] { DEFAULT_GROUP_NAME }));
		expectLastCall().times(2);
		listener.onContactChange(eq(ContactListListener.LIST_CONTACT_ADDED), anyObject(ContactList.class), anyObject(Contact.class));
		expectLastCall();
		replayAll();
		...
		// Second time should not call notifyContactListUpdated, since contact already exists
		contactListManager.approveSubscriptionRequest(TEST_CONTACT);
	...
	private void expectInitialListCreation() {
		listener.onContactChange(eq(ContactListListener.LIST_CREATED), anyObject(ContactList.class), eq((Contact)null));
		expectLastCall();
