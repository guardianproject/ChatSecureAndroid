...
public class SettingActivity extends android.preference.PreferenceActivity implements OnSharedPreferenceChangeListener {
    ...
    private void setInitialValues() {
        ...
        final String pass = Imps.Account.getPassword(cr, mAccountId);
        if (pass != null) mUser.setText(pass);
        text = Imps.Account.getUserName(cr, mAccountId);
        if (text != null) {
        	mUser.setSummary(text);
            mUser.setText(text);
        }
        text = settings.getDomain();
        if (text != null) {
        	mDomain.setSummary(text);
            mDomain.setText(text);
        }
    ...
    public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
    	...
    	String value;
    	if (key.equals(getString(R.string.pref_account_user)) || key.equals(getString(R.string.pref_account_pass))) {
    		ContentResolver cr = getContentResolver();
    		String user = prefs.getString(getString(R.string.pref_account_user), null);
    		final String pass = prefs.getString(getString(R.string.pref_account_pass), null);
    		mAccountId = ImApp.insertOrUpdateAccount(cr, mProviderId, user, pass);
    		if (user != null) mUser.setSummary(user);
    	} else if (key.equals(getString(R.string.pref_account_domain))) {
    		value = prefs.getString(key, null);
    		if (value != null) {
    			settings.setDomain(value);
    			mDomain.setSummary(value);
    		}
    ...
    public void onCreate(Bundle savedInstanceState) {
    	...
    	mUser = (EditTextPreference) findPreference(getString(R.string.pref_account_user));
    	mPass = (EditTextPreference) findPreference(getString(R.string.pref_account_pass));
    	mDomain = (EditTextPreference) findPreference(getString(R.string.pref_account_domain));
